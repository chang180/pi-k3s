...
目前正式環境以公開 demo 為主，不需要建立展示帳號，也不需要引導使用者註冊或登入。

- 主要入口：`/` 與 `/calculate`
- 首頁、頁首與側邊欄已隱藏登入 / 註冊 / dashboard 導航
- HPA 展示重點在 `/calculate` 的分散式計算與 K8s 狀態卡
- Fortify / auth 相關程式仍保留於 starter kit 結構中，但不是目前展示流程的一部分
